...
//
// Emitting it inline in `next.config.mjs` through the static `headers()` table gives only a
// lone `frame-ancestors` directive. That table cannot express a per-request value, which is
// exactly what a script nonce is, so the console shipped with no `script-src`, no `default-src`,
// no `object-src` and no `base-uri` at all: anti-clickjacking only, and nothing else.
//
// Adding a nonce is not a cosmetic upgrade. Next serves its hydration bootstrap as an INLINE
// script carrying the Flight payload, so any `script-src` without a nonce blocks it, `__next_f`
// never fills, React never attaches, and the console renders its controls as dead markup while
// the headers, the type-check, the build and every test stay green.
//
...
function apiOrigin(env) {
  ...
  try {
    ...
    // A relative value (the same-origin reverse-proxy sub-path, e.g. `/agent/api`) is already
    // covered by `'self'`, so it widens nothing and must not be reported as a misconfiguration.
    if (raw.startsWith("/")) return "";
    throw new Error(`NEXT_PUBLIC_API_BASE must be an absolute URL or a path, got: ${raw}`);
  }
}
 ...
 *
 * `src/market_intelligence/api/app.py::_refuse_wildcard` does this for the API surface, and it was the only half that
...
/**
 * Who may frame this console, read in THREE states to match the backend.
 *
 * `src/market_intelligence/api/app.py::_frame_ancestors` reads `MKT_INTEL_FRAME_ANCESTORS` as
 * unset -> `'self'`, set-but-naming-nothing -> `'none'`, otherwise the named origins. The
 * document a browser frames is served by Next, not by the API, so this half has to resolve
 * identically or the two halves of the embedding posture disagree. The middle state is the whole
 * point: a two-state `env.X || "'self'"` reads a deliberately emptied allowlist as consent to
 * same-origin framing, and emitting an EMPTY `frame-ancestors` directive instead is worse still
 * because browsers discard an empty directive as a parse error and the restriction disappears.
 *
 ...
 *
 * A NAMED allowlist cannot be expressed by `X-Frame-Options`, so nothing is sent there rather
 * than a `SAMEORIGIN` that would contradict the CSP in an older agent. Mirrors the service's
