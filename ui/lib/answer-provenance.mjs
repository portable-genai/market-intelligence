...
//
// This console ships no same-origin proxy: it calls the service directly on the API base
// `lib/api.ts` resolves from NEXT_PUBLIC_API_BASE (`<mount>/api` under the portal, the service's
// own origin standalone). So "the console's own API" is that base, not a fixed path, and the
// service exposes both headers (`Access-Control-Expose-Headers`) for the standalone,
// cross-origin case.
...
/**
 * True when `input` addresses this console's own API: the same origin and path prefix as
 * `apiBase`, resolved against the page's own URL.
 *
 * @param {unknown} input what was handed to `fetch`
 * @param {string} here the page's own URL
 * @param {string} apiBase the console's API base, absolute or page-relative
 */
export function isConsoleApi(input, here, apiBase) {
  let raw;
  ...
  try {
    const base = new URL(apiBase.replace(/\/+$/, "") + "/", here);
 ...
 * @param {string} apiBase the console's API base, as `lib/api.ts` resolves it
...
export function watchAnswers(host, apiBase, listener) {
  ...
  if (!installed) {
    ...
    const wrapped = async (input, init) => {
      ...
      if (isConsoleApi(input, host.location.href, apiBase)) {
