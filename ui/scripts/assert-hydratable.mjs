...
// succeeds. The page renders, the headers look correct, and a screenshot looks exactly like a
// working console. What gets shipped is dead markup: `script-src 'self'` blocks Next's inline
...
//
//   1. Every directive the baseline requires is present, and none of them is empty (an empty
//      directive is a CSP parse error browsers discard, which silently removes the restriction).
//   2. The response carries a nonce in `script-src`.
//   3. EVERY `<script>` tag in the document carries that same nonce.
//
// Rule 3 is the one that matters, and it is the one a header assertion cannot express. A statically
// prerendered page was built before the nonce existed, so it emits script tags with no nonce while
// the header advertises one, and because `'strict-dynamic'` disables the `'self'` fallback, that
// combination blocks strictly MORE than the unfixed policy did. The header is byte-identical in the
// working and the broken case. Only the markup knows.
//
...
/** Directives the console's baseline policy must always carry. */
