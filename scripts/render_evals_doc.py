...
GOLDEN = _REPO_ROOT / "eval" / "datasets" / "golden_briefs.jsonl"
...
def _metrics_block() -> list[str]:
    ...
    lines += ["", f"Scored over {len(cases)} golden briefs.", ""]
    return lines
...
def _exercised_block() -> list[str]:
    ...
    service = run_eval._make_service()
    claims = deltas = citations = 0
    for example in examples:
        brief = service.build_brief(
            run_eval.BriefRequest(
                topic=example.topic,
                market=run_eval.Market(example.market),
                vertical=run_eval.Vertical(example.vertical),
            ),
            actor="eval-bot",
            as_of=run_eval._AS_OF,
        )
        claims += len(brief.key_claims)
        citations += len(brief.citations)
        if brief.competitor_analysis is not None:
            deltas += len(brief.competitor_analysis.diff.material_deltas)
    return [
        BLOCKS[1],
        "",
        f"- **{len(examples)} golden briefs** in `eval/datasets/golden_briefs.jsonl`, each with",
        ...
        f"- Those briefs produce **{claims} claims and {deltas} material competitor deltas**,",
        "  which is what `brief_groundedness` is measured over now that it is a claim-level",
        f"  metric, and **{citations} citations**, which is what `citation_accuracy` is measured",
