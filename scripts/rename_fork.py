...
Rewrites the package name, the CLI entry point, the ``MKT_INTEL_`` env-var prefix, and the
...
IdP wiring, brand corpus, eval golden set) that this script does NOT do for you.
"""
...
_OLD_PACKAGE = "market_intelligence"
_OLD_CLI = "mkt-intel"
_OLD_ENV_PREFIX = "MKT_INTEL_"
_OLD_RESOURCE = "market-intelligence"
_OLD_DIST = "market-intelligence"
...
def _rewrite_text(text: str, repls: list[tuple[str, str]]) -> tuple[str, int]:
    ...
    for old, new in repls:
        # For the env prefix, only rewrite ``MKT_INTEL_`` when it prefixes an UPPER token
        # (MKT_INTEL_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource", required=True, help="new resource id stem, e.g. acme-market-intel"
    )
    ...
    print("Then finish the human steps in docs/ADOPTING.md (fixtures, brand corpus, IdP, region).")
    return 0
