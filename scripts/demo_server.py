#!/usr/bin/env python3
"""Live, presenter-controlled demo server for D1 (stdlib only, fully offline).

Holds a real set of D1 services over the in-memory ``local`` stack and reveals one cited
market brief per click, walking the presenter across both verticals (banking + online
retail) and the JP/AU/SG markets. Each step renders the audit-first console reused verbatim
from ``render_brief_ui``. No Google Cloud, no API key, no extra dependencies.

    MKT_INTEL_PROFILE=local PYTHONPATH=src python scripts/demo_server.py [--port 8110]

Then open http://localhost:8110 and click "Next", or drive it with Playwright. The demo
port (8110) is deliberately distinct from the FastAPI API port (8100) and the Next.js
...
import render_brief_ui as r  # sibling: reuse the exact audit-first rendering
...
class DemoSession:
    """Compute the real D1 briefs once, then reveal one per click."""
    ...
    def reset(self) -> None:
        ...
        self.briefs = []
        for topic, market, vertical in scenario._SCENARIOS:
            request = BriefRequest(topic=topic, market=market, vertical=vertical)
            brief = service.build_brief(request, actor="demo", as_of=scenario._AS_OF)
            self.briefs.append(to_jsonable(brief))
        self.idx = 0

    @property
    def at_end(self) -> bool:
        return self.idx >= len(self.briefs) - 1
    ...
    def render(self) -> str:
        data = self.briefs[self.idx]
        return self._inject_controls(r.render_brief(data), data)

    def _inject_controls(self, page_html: str, data: dict) -> str:
        nxt = None if self.at_end else "Reveal the next market brief"
        if nxt:
            next_btn = (
        ...
        label = f"{data.get('market')} / {data.get('vertical')} — {data.get('topic')}"
        bar = (
            "<div class='democtl' data-demo='presenter-step' "
            f"data-step='{self.idx}' data-step-total='{len(self.briefs)}' "
            f"data-step-brief='{r.esc(data.get('id'))}'>"
            f"<span class='lbl'>Step {self.idx + 1}/{len(self.briefs)} — <b>{r.esc(label)}</b></span>"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Live D1 market-intelligence demo server")
    ...
    print(f"D1 demo server on http://{args.host}:{args.port}  (Ctrl-C to stop)")
    try:
