# market-intelligence — Market Intelligence API service image.
#
...
# is unset, so this image sets MKT_INTEL_PROFILE=gcp EXPLICITLY to opt in to the managed stack
# in production (never rely on a baked-in default to select cloud).
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 \
    CMD python -c "import urllib.request,os; urllib.request.urlopen('http://127.0.0.1:'+os.environ.get('PORT','8100')+'/healthz')" || exit 1

CMD exec uvicorn market_intelligence.api.app:app --host 0.0.0.0 --port ${PORT}
