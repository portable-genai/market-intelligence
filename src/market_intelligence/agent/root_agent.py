"""Root ADK agent for the D1 Market Intelligence system, hosted on Agent Runtime.

This is the agent the Gemini Enterprise Agent Platform **Agent Runtime** (ex-Agent Engine)
hosts. It wires together:

* the market-intelligence domain-service :class:`FunctionTool` wrappers (``agent.tools``:
  ``build_market_brief``, ``competitor_analysis``),
* the isolated ``google_search`` market-research **sub-agent** as an ``AgentTool``
  (``agent.grounding_agent``; one built-in tool per agent, SPEC §3), gated on
  ``settings.grounding_enabled``,
* an optional governed **MCP** toolset (``MCPToolset``) when ``MKT_INTEL_MCP_SERVER_URL`` is
  set, so the same least-privilege catalog declared in ``adapters/gcp/mcp_tool_catalog.py``
  can be served from an out-of-process MCP server (rule R4),
...
local / on-prem / test profile imports it cleanly).
...
ROOT_AGENT_NAME = "market_intelligence_agent"
MCP_SERVER_URL_ENV = "MKT_INTEL_MCP_SERVER_URL"

_ROOT_INSTRUCTION = (
    "You are the D1 Market Intelligence agent for a bank or online retailer's marketing "
    "strategy team. You produce cited market briefs and competitor analysis, generic across "
    "banking and online retail and the JP / AU / SG markets.\n\n"
    "Routing:\n"
    "- 'Brief me on <topic>' or a full market-read request -> call build_market_brief.\n"
    "- 'How are competitors moving on <area>?' / SWOT / where-to-play -> call "
    "competitor_analysis.\n"
    "- Need recent public-web corroboration -> delegate to the market_research_grounding "
    "sub-agent; treat its results as secondary evidence only.\n\n"
    "Rules:\n"
    "- Every claim must carry a citation to its source (research finding, internal corpus, "
    "competitor snapshot). Never invent a figure, a competitor move, a trend score or a "
...
def build_root_agent(settings: Settings | None = None) -> LlmAgent:
    ...
    Wires the market-intelligence FunctionTools, the optional ``google_search`` grounding
    sub-agent (as an ``AgentTool``), an optional governed ``MCPToolset``, and the
    ...
    sub_agent = build_grounding_agent(settings)
    if sub_agent is not None:
        tools.append(AgentTool(agent=sub_agent))

    # Optional out-of-process governed MCP toolset (rule R4). Only wired when an MCP server
    # URL is configured; the in-process FunctionTools above are the default surface.
    ...
    return LlmAgent(
        ...
        description=(
            "Market-intelligence agent: builds cited market briefs and competitor analysis "
            "(segment read, competitor moves, trend scores, SWOT / where-to-play) for a bank "
            "or online retailer across the JP / AU / SG markets."
        ),
...
# ADK renamed its SSE/HTTP connection-params class across releases; resolve whichever this
# ADK exposes so the wiring survives a version bump (the toolset itself is stable).
...
def _build_mcp_toolset() -> Any | None:
    """Build a governed ``MCPToolset`` from ``MKT_INTEL_MCP_SERVER_URL``, or ``None``.

    Realizes the wiring the ``mcp_tool_catalog`` adapter documents: the governed,
    least-privilege catalog declared in ``adapters/gcp/mcp_tool_catalog.py`` (MCP 2026-07-28)
    is served by an out-of-process MCP server, and the agent reaches it through an
    ``MCPToolset``. Offline there is no MCP server, so the in-process FunctionTools are the
    surface and this returns ``None``. ADK / MCP imports are lazy (SPEC §4).
    """
...
class _LazyRootAgent:
    ...
    be importable at module import time (local / on-prem / test profile, SPEC §4). The real
