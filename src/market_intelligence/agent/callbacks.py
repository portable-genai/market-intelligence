...
The domain service already screens INPUT and OUTPUT and audits inside its pipeline
(SPEC §5). These ADK callbacks add a **second, independent line of defence at the model
boundary**: every prompt that reaches the LLM and every response that leaves it is, once
more,
  ...
  2. **audited** (Cloud Logging locked WORM bucket via :class:`AuditSinkPort`) with a record
     at agent turn end (P-07).

D1 processes public + aggregate market data and no customer PII (C2/C3/C4 n/a in the
practices audit), so there is no PII-redaction callback here (contrast the document
verticals, which redact customer data before the model). The guardrail's own
``sanitized_text`` is still honoured, so any content the policy chooses to mask never
reaches the model or the audit record.

The callbacks are built from a :class:`~market_intelligence.config.Container`, so the active
profile decides whether these are real Model Armor / Cloud Logging calls or on-prem
placeholders.

Span privacy: ADK can attach message content to trace spans. :func:`configure_span_privacy`
sets ``ADK_CAPTURE_MESSAGE_CONTENT_IN_SPANS=false`` (idempotent; never overrides an operator
who has already pinned it). ADK imports are done lazily inside the factory / callbacks so
this module imports without ADK installed (SPEC §4).
"""
...
def configure_span_privacy() -> None:
    ...
    pinned it. Pairs with the Cloud Trace adapter's content-capture-off setting (SPEC §3).
    """
