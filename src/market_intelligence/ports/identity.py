...
other port, and is the single seam where a client-asserted actor is replaced by a
server-verified one.

The Protocol itself is NOT written out here. It comes from ``hex_service_kit.identity``, so
there is exactly one definition of it in the fleet and ``tests/contract/test_port_parity.py``
checks every adapter against that one rather than against a local look-alike. A structural
``isinstance`` check passes for a copy; only object identity catches the drift.
