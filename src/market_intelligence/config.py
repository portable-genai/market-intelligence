...
D1 is generic and APAC: the active ``vertical`` (banking | online retail) and ``market``
...
#: The port whose ACTIVE binding decides what the model pill says before any answer.
...
class ProfileChoice:
    """The ONE resolution of ``MKT_INTEL_PROFILE``, and what each consumer must key off.
    ...
    profile: str = "local"
    #: Was the profile named DELIBERATELY (``MKT_INTEL_PROFILE`` set, or a profile written into
...
class DeepResearchSettings:
    ...
    max_sources: int = 12
    grounding_tool: str = "google_search"


@dataclass(frozen=True)
class KnowledgeBaseSettings:
    """The internal research / brand corpus (File Search / A2)."""

    base_url_env: str = "KNOWLEDGE_BASE_URL"
    data_store_id: str = "mkt-research-corpus"  # only used by the standalone GCP adapter
...
GUARDRAIL_ENV = "MKT_INTEL_GUARDRAIL"
REVIEW_ROUTING_ENV = "MKT_INTEL_REVIEW_ROUTING"
...
#: rather than a hand-off that fails, silently, on every brief.
...
class LoggingSettings:
    log_name: str = "market-intelligence-audit"
    bucket: str = "market-intelligence-worm"
...
class LocalSettings:
    ...
    Empty strings select the per-package default under ``~/.market_intelligence/``; tests
    pass ``:memory:`` for ephemeral, deterministic stores. No Google Cloud here.
    """

    db_path: str = ""  # SQLite FTS5 research corpus index
    audit_path: str = ""  # append-only audit store
    seed_path: str = ""  # research-corpus seed JSON ("" => bundled fictional seed)


@dataclass(frozen=True)
class PolicySettings:
    """Adopter-owned deterministic-engine policy with reference defaults."""

    claim_similarity_threshold: float = 0.6
    claim_min_tokens: int = 1
    trend_half_life_days: float = 30.0
    trend_recent_window_days: int = 30
    trend_rising_band: float = 0.6
    trend_fading_band: float = 0.3
    swot_attractiveness_weight: float = 0.6
    swot_right_to_win_weight: float = 0.4
    swot_opportunity_trend_score: float = 0.5
    swot_max_options: int = 5

    def __post_init__(self) -> None:
        swot_values = (
            self.swot_attractiveness_weight,
            self.swot_right_to_win_weight,
            self.swot_opportunity_trend_score,
        )
        if not all(0 <= value <= 1 for value in swot_values):
            raise ValueError("SWOT weights and thresholds must be within 0..1")
        if abs(self.swot_attractiveness_weight + self.swot_right_to_win_weight - 1) > 1e-9:
            raise ValueError("SWOT ranking weights must sum to 1")
        if not 0 <= self.trend_fading_band < self.trend_rising_band <= 1:
            raise ValueError("trend bands must satisfy 0 <= fading < rising <= 1")
        if self.trend_half_life_days <= 0 or self.trend_recent_window_days <= 0:
            raise ValueError("trend windows must be positive")
        if not 0 <= self.claim_similarity_threshold <= 1 or self.claim_min_tokens < 1:
            raise ValueError("claim dedup policy is outside its supported range")
        if self.swot_max_options < 1:
            raise ValueError("swot_max_options must be positive")
...
class Settings:
    ...
    def load(path: str | os.PathLike[str] | None = None) -> Settings:
        ...
        nested: dict[str, Any] = {
            ...
            "deep_research": DeepResearchSettings(**(raw.pop("deep_research", {}) or {})),
