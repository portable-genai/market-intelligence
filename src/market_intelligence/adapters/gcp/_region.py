"""Residency-region validation for the GCP adapter family (D1).

D1 supports the three APAC markets (JP, AU, SG) and each carries a residency region
...
:data:`market_intelligence.domain.models.MARKET_PROFILES`, which is itself config + seed
...
def allowed_regions(settings: Settings) -> frozenset[str]:
    """The set of residency regions D1 is permitted to use, from the market profiles."""
...
def resolve_region(settings: Settings, market: Market | None = None) -> str:
    ...
    if region not in permitted:
        ...
        raise UnsupportedMarketError(
            f"Residency region '{region}' is not in the permitted set for D1 ({allowed}). "
