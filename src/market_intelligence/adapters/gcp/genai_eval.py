"""Gen AI evaluation gate adapter (EvaluationGatePort) — the A4 promotion gate for D1.

Backs the domain ``EvaluationGatePort`` with the **Gen AI evaluation service**, accessed
through ``vertexai.Client(project, location).evals``. Over a golden dataset of briefs it
scores the system on the metrics that matter for a cited market-intelligence system
(brief groundedness, citation accuracy, competitor-diff accuracy, review safety) and maps
