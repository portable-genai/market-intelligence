"""Gemini LLM adapter (LlmPort) — GCP managed stack.
...
In D1 the LLM only narrates and drafts over the already-computed deterministic result (the
diff, the trend scores, the SWOT and the ranking). It never decides the numbers. The
