"""A2A registry adapter (AgentRegistryPort) — agent discovery and governance for D1 (A3).

Backs the domain ``AgentRegistryPort`` with an in-process, **A2A v1.0**-style registry of
:class:`AgentCard` objects. In a standalone deployment D1 registers its own card here and
...
# D1's own skills, surfaced on its AgentCard so peers / the registry can discover the
# governed market-intelligence capabilities the system offers (generic across verticals).
_D1_SKILLS: tuple[AgentSkill, ...] = (
    AgentSkill(
        id="market_brief",
        name="Cited market brief",
        description=(
            "Build a cited market brief for a topic from grounded deep research and the "
            "internal corpus, for any of banking / online retail across JP/AU/SG."
        ),
    ),
    AgentSkill(
        ...
        description=(
            ...
            "with provenance on every finding."
        ),
    ),
)
...
class A2ARegistryAdapter:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        # Seed the registry with D1's own card so a standalone deployment is discoverable.
    ...
    def agent_card_dict(self, name: str | None = None) -> dict:
        """Return the ``/.well-known/agent-card.json`` body for ``name`` (default: D1's)."""
    ...
    def _self_name(self) -> str:
        return self._settings.agent_engine.display_name or "market-intelligence"

    def _self_card(self) -> AgentCard:
        return AgentCard(
            name=self._self_name(),
            description=(
                "D1 Market Intelligence and Competitor Analysis — grounded deep research + "
                "deterministic competitor-move diff and SWOT, generic across banking and "
                "online retail and the JP/AU/SG markets, with cited findings."
            ),
            url=f"https://market-intelligence.{self._settings.region}.example/a2a",
            version="1.0.0",
            skills=_D1_SKILLS,
