...
class IapIdentityAdapter:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        # "/projects/<NUM>/apps/<ID>". Configure via MKT_INTEL_IAP_AUDIENCE; required in
        # secure mode. Unset and set-but-empty DELIBERATELY collapse here, because both are
        # closed in the SAME direction: the empty string names no audience, and :meth:`resolve`
        # refuses to verify an assertion without one. There is no permissive default to
        # inherit, so ``read_env_setting(...).value`` is the whole decision.
        self._audience = read_env_setting("MKT_INTEL_IAP_AUDIENCE").value
    ...
    def resolve(self, ctx: RequestContext) -> Principal:
        ...
        if not self._audience:
            raise IapAudienceUnconfiguredError(
                "MKT_INTEL_IAP_AUDIENCE is not configured, so no IAP assertion can be verified "
                "and this deployment can authenticate nobody. Verifying WITHOUT an "
