"""Cloud Logging WORM audit adapter (AuditSinkPort) — GCP managed stack for D1.
...
routes the ``market-intelligence-audit`` log to a **locked log bucket** (WORM, ~7-year
retention), so records are write-once and immutable: the regulator-grade audit guarantee
(A5).
