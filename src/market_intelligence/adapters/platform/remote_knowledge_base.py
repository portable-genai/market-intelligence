...
When D1 reuses the shared platform, the internal corpus is the **A2 Enterprise Knowledge
Base**. This adapter implements the port by POSTing to A2's ``/v1/search`` (base URL from
``KNOWLEDGE_BASE_URL``). Constructs cleanly with no Google Cloud SDK; the HTTP body is wired in the
platform phase.
"""
...
class RemoteKnowledgeBaseError(MarketIntelError):
