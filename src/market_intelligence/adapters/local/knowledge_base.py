"""Local knowledge-base adapter (KnowledgeBasePort) — SQLite FTS5 internal corpus.

The ``local`` profile's stand-in for **File Search / Agent Search** over the internal
research / brand corpus (whose GCP adapter is Agent Search, and whose platform adapter is
the shared A2 Enterprise KB): a ``sqlite3`` database with an **FTS5** virtual table over
the seeded fictional research passages, queried with BM25 (``ORDER BY rank``). It is
SDK-free, deterministic and seedable, so the same code grounds the offline CLI run and the
unit tests. There is no Google emulator for Agent Search, so this path is unconditional.
...
_DEFAULT_DB_DIR = Path.home() / ".market_intelligence"
...
class LocalKnowledgeBaseAdapter:
    """Index + retrieve internal-corpus passages from a local SQLite FTS5 store (BM25)."""
