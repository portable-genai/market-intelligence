...
# local -> adapters -> market_intelligence -> src -> <repo>.
...
class LocalOfflineEvalAdapter:
    ...
    def _load_run_eval() -> ModuleType | None:
        if not _EVAL_SCRIPT.exists():
            return None
        spec = importlib.util.spec_from_file_location("mkt_local_run_eval", _EVAL_SCRIPT)
        if spec is None or spec.loader is None:
            return None
        module = importlib.util.module_from_spec(spec)
        sys.modules.setdefault("mkt_local_run_eval", module)
