...
persona) without standing up any identity provider. It is bound ONLY under the laptop
profiles (``local`` and ``live``); secure mode uses the IAP adapter, which verifies a real
assertion.

These personas are an UNAUTHENTICATED grant of a tenant identity, so this adapter refuses to
construct unless a laptop profile was chosen deliberately: the profile must actually be
``local`` or ``live`` AND ``MKT_INTEL_PROFILE`` (or the reviewed settings file) must have
named it rather than it being inherited from a fallback. A missing env var therefore fails
closed instead of handing out a demo identity.
"""
...
# D1's market-intelligence domain (analyst / approver / marketing / audit); the personas
# span two tenants so cross-tenant authorization is demoable offline.
_PERSONAS: tuple[Principal, ...] = (
    Principal(
        subject="demo.analyst@brand.example",
        principals=("group:mi-analyst", "group:marketing"),
        tenant="demo-brand",
        assurance="local-demo",
        source="local-persona:analyst",
    ),
    Principal(
        subject="demo.approver@brand.example",
        principals=("group:mi-analyst", "group:marketing", "group:mi-approver"),
        tenant="demo-brand",
        assurance="local-demo",
        source="local-persona:approver",
    ),
    Principal(
        subject="demo.auditor@brand.example",
...
class LocalPersonaIdentityAdapter:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        if not settings.profile_explicit:
            raise LocalPersonaProfileError(
                "MKT_INTEL_PROFILE is not set, so the local profile was inherited rather than "
                "chosen; the seeded dev personas grant a tenant identity with no authentication "
                "and are refused. Set MKT_INTEL_PROFILE=local deliberately for a dev or demo "
                "run, or MKT_INTEL_PROFILE=gcp for a real deployment."
            )
