...
(``MKT_INTEL_GUARDRAIL``, ``MKT_INTEL_REVIEW_ROUTING``), the container binds one of these
...
**Recording wrapper.** Every caller that hands a brief to the router (the API route, the agent
tool, the MCP ``deep_research`` tool and the CLI) wraps the bound router in
...
because an already-audited brief must not fail on a console outage, but it is no longer
invisible: the caller reports ``failed``. A competitor analysis requires review but is never
handed to the router, so it reports nothing.
...
from ..domain.models import Direction, GuardrailVerdict, MarketBrief
...
class RecordingReviewRouter:
    ...
    def route(self, brief: MarketBrief, *, maker: str, tenant: str = "") -> None:
        if not getattr(self._inner, "enabled", True):
            ...
            return
        try:
            self._inner.route(brief, maker=maker, tenant=tenant)
