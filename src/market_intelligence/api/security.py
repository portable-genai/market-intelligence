...
client-supplied actor is ignored entirely: the audit actor (and the entitlement principals
a governed-retrieval path would consume) flow from here, closing the spoofable-identity
gap. A failure to resolve a verified principal is a 401.
...
def get_principal(request: Request) -> Principal:
    ...
    construct when the local profile was inherited from a fallback rather than chosen, and that
    refusal must land as a 401 rather than a 500.
    """
