...
from market_intelligence import ports
from market_intelligence.config import Settings
from market_intelligence.domain.errors import UnsupportedMarketError
from market_intelligence.domain.models import Market
...
GCP_ADAPTERS: dict[str, tuple[str, type]] = {
    "research": (
        "market_intelligence.adapters.gcp.deep_research:GeminiDeepResearchAdapter",
        ports.ResearchPort,
    ),
    "knowledge_base": (
        "market_intelligence.adapters.gcp.file_search_kb:FileSearchKnowledgeBaseAdapter",
        ports.KnowledgeBasePort,
    ),
    "llm": (
        "market_intelligence.adapters.gcp.gemini_llm:GeminiLLMAdapter",
        ports.LlmPort,
    ),
    "guardrail": (
        "market_intelligence.adapters.gcp.model_armor_guardrail:ModelArmorGuardrailAdapter",
        ports.GuardrailPort,
    ),
    "audit": (
        "market_intelligence.adapters.gcp.cloud_logging_audit:CloudLoggingAuditAdapter",
        ports.AuditSinkPort,
    ),
    "tracer": (
    ...
    "evaluation": (
        "market_intelligence.adapters.gcp.genai_eval:GenAiEvalAdapter",
        ports.EvaluationGatePort,
    ),
    "agent_registry": (
        "market_intelligence.adapters.gcp.a2a_registry:A2ARegistryAdapter",
        ports.AgentRegistryPort,
    ),
    "tool_catalog": (
        "market_intelligence.adapters.gcp.mcp_tool_catalog:McpToolCatalogAdapter",
...
def test_gcp_adapter_constructs_and_satisfies_protocol(port_name: str):
    ...
    from market_intelligence.config import instantiate
...
def test_region_validation_accepts_each_apac_market():
    from market_intelligence.adapters.gcp._region import resolve_region
