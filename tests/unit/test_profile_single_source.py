...
``MKT_INTEL_PROFILE`` must never be read with a ``"local"`` fallback, and two places could read
it: the Python loader and the shipped ``config/settings.yaml`` interpolation token. Guarding
either alone leaves the other re-deriving the same decision, which is how an unset variable
becomes readable as consent to the no-auth posture. A drift guard is therefore part of the
defence: any module (or the settings file) that re-derives the profile with its own permissive
default can reintroduce the whole class, so only ``config.resolve_profile`` may decide it.
"""
...
_SRC = _ROOT / "src" / "market_intelligence"
_CONFIG = _SRC / "config.py"
_SETTINGS_FILE = _ROOT / "config" / "settings.yaml"
_PROFILE_TOKEN = re.compile(r"\$\{MKT_INTEL_PROFILE(?::-(.*?))?\}")
...
def test_only_the_resolver_reads_the_profile_variable_from_the_environment() -> None:
    ...
    assert not offenders, (
        ...
        "so an unset MKT_INTEL_PROFILE can again be read as consent:\n" + "\n".join(offenders)
    )
...
def test_the_shipped_settings_file_supplies_no_permissive_profile_default() -> None:
    ...
    assert not defaults, (
        "config/settings.yaml gives MKT_INTEL_PROFILE a fallback of "
...
def test_an_EMPTIED_variable_refuses_rather_than_inheriting_the_unset_default() -> None:
    ...
    """
    for environ in ({"MKT_INTEL_PROFILE": ""}, {"MKT_INTEL_PROFILE": "   "}):
...
def test_a_deliberate_profile_is_carried_through_unchanged() -> None:
    choice = resolve_profile({"MKT_INTEL_PROFILE": "gcp"})
...
def test_an_unknown_or_mis_capitalised_profile_is_refused(value: str) -> None:
    with pytest.raises(ValueError, match="MKT_INTEL_PROFILE"):
        resolve_profile({"MKT_INTEL_PROFILE": value})
