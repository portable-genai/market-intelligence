...
in the runbook table, so ``MKT_INTEL_FRAME_ANCESTORS="*"`` shipped
``Content-Security-Policy: frame-ancestors *``, which permits ANY page anywhere to frame the
console, and ``MKT_INTEL_CORS_ORIGINS="*"`` handed the same blanket trust to the CORS
...
_CORS_ENV = "MKT_INTEL_CORS_ORIGINS"
...
def test_every_wildcard_spelling_is_refused_in_either_list(
    ...
    ``null`` was the hole. ``MKT_INTEL_FRAME_ANCESTORS=null`` resolved to the string ``null``
    and shipped ``Content-Security-Policy: frame-ancestors null``, and
    ``MKT_INTEL_CORS_ORIGINS=null`` put ``null`` in the credentialed CORS allowlist. A
