...
TWO_STATE_READS_WITH_A_REASON: dict[str, str] = {
    ...
    "DEMO_URL": (
        ...
        "attaches to instead of spawning one. Presenter ergonomics on loopback; the demo server "
        "binds 127.0.0.1 and is not configurable."
    ),
    "PORT": (
        "api/app.py main(): the TCP port the local uvicorn runner listens on, supplied by the "
        "platform in a container. It carries no posture at all: WHICH interface is bound is "
        "decided by resolve_bind_host from the bind profile, and an emptied value fails int() "
        "loudly rather than opening anything."
    ),
    "SLOWMO_MS": (
        "scripts/demo_playwright.py: how long Playwright pauses between steps so a human can "
        "follow the walkthrough. Pure presentation pacing; it reaches no service and grants "
        "nothing."
    ),
}
