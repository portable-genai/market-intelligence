...
that supplies the audit actor server-side (never a client-asserted value).
"""
...
from market_intelligence.adapters.local.identity import LocalPersonaIdentityAdapter
from market_intelligence.adapters.onprem.identity import OnPremIdentityAdapter
from market_intelligence.config import Settings
from market_intelligence.domain.identity import IdentityError, RequestContext
...
def test_default_persona_when_no_header() -> None:
    ...
    assert principal.subject == "demo.analyst@brand.example"
...
def test_persona_selected_by_header() -> None:
    ...
    assert principal.subject == "demo.auditor@brand.example"
    assert principal.principals == ("group:audit",)


def test_persona_header_is_case_insensitive() -> None:
    # RequestContext.header() lower-cases the lookup name, and the API builds the context
    # with lower-cased header keys, so a host that sends X-Dev-Persona still resolves.
    principal = _adapter().resolve(RequestContext(headers={"x-dev-persona": "other-tenant"}))
    assert principal.tenant == "other-brand"
...
def test_personas_are_refused_when_the_local_profile_was_never_chosen() -> None:
    """Unset MKT_INTEL_PROFILE is no choice, so the no-auth personas are not handed out."""
    with pytest.raises(IdentityError, match="MKT_INTEL_PROFILE"):
        LocalPersonaIdentityAdapter(Settings(profile="local", profile_explicit=False))


def test_personas_are_refused_outside_the_laptop_profiles() -> None:
    with pytest.raises(IdentityError, match="laptop-profile only"):
