...
from market_intelligence.adapters.local.audit import LocalAppendOnlyAuditAdapter
from market_intelligence.config import LocalSettings, Settings
from market_intelligence.domain.models import AuditEvent, Decision
...
def test_events_round_trip_and_chain_verifies() -> None:
    adapter = _adapter()
    adapter.record(_event("market_brief"))
    adapter.record(_event("research"))
    events = adapter.read_all()
    assert [e["action"] for e in events] == ["market_brief", "research"]
...
def test_tampering_is_detected() -> None:
    ...
    conn.execute(
        "UPDATE audit_log SET event_json = replace(event_json, 'market_brief', 'x') WHERE seq = 1"
    )
    conn.commit()
