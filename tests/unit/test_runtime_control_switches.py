...
needs; and every caller that hands a brief to the review router says what happened to it.
...
from market_intelligence.adapters.controls import (
    DisabledGuardrail,
...
from market_intelligence.envread import ConfiguredEmptyError
...
def test_a_process_with_a_control_off_says_so_at_startup(
    ...
    with caplog.at_level(logging.WARNING, logger="market_intelligence.config"):
...
# --------------------------------------------------------------------------- #
class _Accepting:
    def route(self, brief: object, *, maker: str, tenant: str = "") -> None:
        return None
...
def test_a_failed_hand_off_is_reported_and_logged_never_raised(
    ...
    failed = RecordingReviewRouter(_Refusing())
    with caplog.at_level(logging.WARNING, logger="market_intelligence.adapters.controls"):
        failed.route(object(), maker="m")  # type: ignore[arg-type]
    assert failed.outcome is ReviewRouting.FAILED
...
# --------------------------------------------------------------------------- #
# Every caller reports what happened to the brief it handed off
# --------------------------------------------------------------------------- #
_BRIEF_BODY: dict[str, Any] = {
    "topic": "savings and account fees",
    "market": "SG",
    "vertical": "banking",
}
...
def _post_brief(monkeypatch: pytest.MonkeyPatch, container: Container) -> dict[str, Any]:
    monkeypatch.setattr(deps, "get_container", lambda: container)
    response = TestClient(app, client=LOOPBACK_PEER).post("/v1/brief", json=_BRIEF_BODY)
    assert response.status_code == 200, response.text
    body: dict[str, Any] = response.json()
    return body


def test_the_api_reports_a_routed_brief(
    monkeypatch: pytest.MonkeyPatch, local_settings: Settings
) -> None:
    body = _post_brief(monkeypatch, _container(local_settings))
...
def test_the_api_reports_routing_off(
    ...
    body = _post_brief(monkeypatch, _container(local_settings, review_routing=False))
    assert body["review_routing"] == "off"


def test_the_api_reports_a_failed_hand_off_instead_of_failing_the_brief(
    monkeypatch: pytest.MonkeyPatch, local_settings: Settings
) -> None:
    body = _post_brief(monkeypatch, _container(local_settings, router=_Refusing()))
...
def test_the_agent_tool_reports_the_hand_off(local_settings: Settings) -> None:
    ...
    payload = tools.build_market_brief("savings and account fees", settings=settings)
    assert payload["review_routing"] == "off"


def test_the_mcp_deep_research_tool_reports_the_hand_off(
    monkeypatch: pytest.MonkeyPatch, local_settings: Settings
) -> None:
    from market_intelligence.mcp import server as mcp_server

    monkeypatch.setattr(
        deps, "get_container", lambda: _container(local_settings, router=_Refusing())
    )
    payload = mcp_server.build_handlers(actor="svc:test")["deep_research"](
        topic="savings and account fees", market="SG", vertical="banking"
    )
    assert payload["review_routing"] == "failed"


def test_the_cli_says_where_the_brief_went(
    monkeypatch: pytest.MonkeyPatch, local_settings: Settings
) -> None:
    from market_intelligence.cli.main import app as cli

    monkeypatch.setattr(
        deps, "get_container", lambda: _container(local_settings, review_routing=False)
    )
    result = CliRunner().invoke(cli, ["brief", "savings and account fees"])
