...
shipped ``Dockerfile`` CMD is ``exec uvicorn market_intelligence.api.app:app --host 0.0.0.0``,
which never reaches it. Executed before the guard existed: a LAN peer got ``200`` on
...
The control is the third pair: a VERIFYING identity binding stands the guard down, so
...
_INSECURE_DEMO_ENV = "MKT_INTEL_ALLOW_INSECURE_DEMO"
...
def test_the_shipped_entry_point_serves_the_app_object_not_main() -> None:
    ...
    assert "market_intelligence.api.app:app" in dockerfile
...
def test_a_verifying_identity_binding_stands_the_guard_down(
    ...
    # ... and it serves no seeded personas to anybody, because that binding is not bound.
    assert TestClient(module.app, client=LAN_PEER).get("/v1/personas").json() == []
