...
def test_the_health_contract_carries_both_halves() -> None:
    ...
    """
    from market_intelligence.api.schemas import HealthModel
...
def test_not_implemented_is_claimed_only_by_an_adapter_that_never_calls_a_model() -> None:
    ...
    if settings.generator_model != "managed-not-implemented":
        return
    from market_intelligence.config import _GENERATOR_PORT
...
def _code_only(source: str) -> str:
    """``source`` with its comments removed, so a rule cannot be satisfied by prose about it."""
    import re as _re
...
def test_the_console_shows_the_model_that_answered_as_pills_not_a_banner() -> None:
    ...
    model configuration would call rather than the one that answered. The chain is held here from
    the offline gate: the pills start from `/healthz`, read both answer headers through the one
    fetch wrapper, and are mounted in the layout. `ui/tests/answer-provenance.test.mjs` proves
    the wrapper.

    This console ships no same-origin route handler, so the pills must read the SAME base the
    rest of the console reads (`API_BASE` from `lib/api.ts`). The banner once fetched
    `/api/agent/healthz`, a path this console never served, and rendered nothing on every page
    load: a check that can only fail by disappearing is exactly what no reviewer notices.
    """
    ui = Path("ui")
    assert not (ui / "app" / "api" / "agent").exists(), (
        "this console now ships a same-origin route handler: the pills must read through it, "
        "and it must forward x-answered-by and x-search-used"
    )
    pills = _code_only((ui / "app" / "ModelPills.tsx").read_text(encoding="utf-8"))
    assert "`${API_BASE}/healthz`" in pills, "the pills do not start from the console's /healthz"
    ...
    assert "prefers-color-scheme: dark" not in css, (
        "this console is light-only; a dark-scheme pill override inverts the pills on a light page"
    )
