...
from market_intelligence import config, ports
from market_intelligence.config import LocalSettings, Settings, instantiate
...
def test_port_protocols_matches_settings_adapters():
    ...
    with a green CI (silent drift), and equally a stale PORT_PROTOCOLS entry whose binding was
    removed goes unnoticed. This reverse set-equality fails loudly on drift in either
    direction.
    """
...
def test_onprem_research_fails_fast():
    ...
    adapter = instantiate(settings.adapters["research"]["onprem"], settings)
    with pytest.raises(NotImplementedError):
        adapter.research(ResearchQuery(topic="x", market=Market.SG, vertical=Vertical.BANKING))


def test_local_research_returns_real_sources():
    """The local stack is WORKING: research returns real, cited sources offline."""
    from market_intelligence.domain.models import Market, ResearchQuery, Vertical

    settings = _settings("local")
    adapter = instantiate(settings.adapters["research"]["local"], settings)
    result = adapter.research(
        ResearchQuery(topic="savings", market=Market.SG, vertical=Vertical.BANKING)
    )
    assert result.sources, "local research returned no sources for the seeded corpus"
...
def test_shared_types_are_the_commons_objects_not_local_look_alikes():
    """Object IDENTITY, which is the only assertion that can see this class of drift.

    Every structural check in this file passes for a hand-copied look-alike: ``isinstance``
    against a ``runtime_checkable`` Protocol is satisfied by shape, and a redeclared dataclass
    with the same three fields satisfies every constructor and every attribute read. That is
    exactly how sixteen repositories each ended up with their own ``TokenUsage``,
    ``EvalReport`` and tracer Protocol, and how those copies drifted: one dropped the
    evaluation port, two dropped its ``gate`` method, one changed an audit return type.

    ``is`` cannot be satisfied by a copy. If someone reintroduces a local declaration, this
    fails immediately and names the module that did it.
    """
    ...
    from market_intelligence.domain import identity as domain_identity
    from market_intelligence.domain import models
