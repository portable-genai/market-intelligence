...
sibling `human-review-console` (rule R8), never to auto-execution.
...
guardrail screening around the LLM is the sibling `agent-guardrail-gateway`, consumed rather than
re-implemented.
...
model documentation are the sibling `model-quality-gate` system; this repo's gate mirrors its thresholds
...
local counsel; the `market-intelligence`-control column is stable across regulators. At scale, the sibling
`compliance-advisory` and its control-mapping module (`domain/control_mapping/`)
...
The residency-violation CI gate is the sibling `architecture-validator` (`domain/residency/`); the exit/concentration-risk plan is `operational-resilience-mapping` (`domain/concentration_exit/`). This repo enforces residency
