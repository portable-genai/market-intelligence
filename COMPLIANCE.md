# COMPLIANCE: `market-intelligence` Market Intelligence and Competitor Analysis

This maps every General Principle (P-01..P-13) and dependency rule (R1..R8) to a concrete
control in **this** repo. Where a principle does not apply to `market-intelligence`, it is marked **n/a** with
the reason. `market-intelligence` processes public and aggregate market data and **no customer PII**, so its
...
| # | Principle | How `market-intelligence` implements it | Evidence |
|---|-----------|----------------------|----------|
...
| **P-04** | Minimise data to the model | `market-intelligence` sends public and aggregate market data only, no customer PII; the model-boundary callback still guardrail-screens every prompt and response, and spans capture no content | `agent/callbacks.py`, `domain/brief_service.py` |
| **P-05** | Grounding over fine-tuning | Briefs are grounded on the internal research corpus (`enterprise-knowledge-base` File Search) plus deep research; no training on data | `ports/knowledge_base.py`, `ports/research.py`, `adapters/*/file_search_kb*`, `adapters/*/deep_research*` |
| **P-06** | Human-in-the-loop / maker-checker | Every `MarketBrief` and `CompetitorAnalysis` is `requires_human_review=True`; a qualified strategist disposes before anyone acts; the escalation is ROUTED to the `human-review-console` maker-checker console (rule R8), not left as a boolean | `domain/brief_service.py`, `domain/models.py`, `ports/review_router.py`, `adapters/*/review_router.py` |
...
| **P-08** | Eval-gated promotion | Offline eval gate scores brief groundedness, citation accuracy, diff accuracy and review safety; `model-quality-gate` at promotion | `eval/run_eval.py`, `ports/observability.py` (`EvaluationGatePort.gate`) |
...
| **P-13** | Fair, consented marketing (advertising compliance) | `market-intelligence` produces internal strategy artifacts (briefs, competitor analysis), not customer-facing marketing, so it publishes no advertising itself; any output that becomes customer-facing must pass `marketing-compliance-gate` (rule R7). The agent instruction forbids drafting customer-facing ad copy here | `agent/root_agent.py` instruction, R7 below |

---

## Dependency rules

`market-intelligence`'s mandatory dependencies are **`enterprise-knowledge-base`, `agent-registry`, `model-quality-gate` (gate) and `agent-observability`** (see `systems/`). Each
...
| Rule | Requirement | How `market-intelligence` satisfies it | Evidence |
|------|-------------|---------------------|----------|
| **R1** | Customer PII handling: `agent-guardrail-gateway` + DLP redaction | **n/a for PII** (`market-intelligence` handles public + aggregate market data, no customer PII: C2/C3/C4 are n/a in the practices audit). The `agent-guardrail-gateway` is still consumed for prompt-injection and unsafe-output screening at both the pipeline and the model boundary | `ports/safety.py`, `domain/brief_service.py`, `agent/callbacks.py` |
| **R2** | Audit to `agent-observability` | Every interaction writes an immutable WORM `AuditEvent`; the `platform` adapter posts to `agent-observability` `/v1/audit` | `adapters/gcp/cloud_logging_audit.py`, `adapters/platform/remote_audit.py` |
| **R3** | Governed RAG via `enterprise-knowledge-base` | The internal research / brand corpus is retrieved via `enterprise-knowledge-base` governed File Search | `ports/knowledge_base.py`, `adapters/platform/remote_knowledge_base.py` |
...
| **R6** | Validated by `architecture-validator` at intake | As a new project, `market-intelligence` is validated by the `architecture-validator` intake validator externally. n/a in-repo (`architecture-validator` is the validator, not a `market-intelligence` runtime dependency) | intake handled by `architecture-validator` externally |
| **R7** | Marketing compliance via `marketing-compliance-gate` | `market-intelligence` produces internal strategy artifacts, not customer-facing marketing. Any output that becomes customer-facing must pass `marketing-compliance-gate` (per-market advertising / consumer-protection claim check, brand guidelines, marketing consent) and screen via `agent-guardrail-gateway`. The agent forbids drafting customer-facing copy here | `agent/root_agent.py` instruction; `marketing-compliance-gate` governance |
| **R8** | Route `requires_human_review` to `human-review-console` | Every escalated brief is submitted to the `human-review-console` Human-Review & Maker-Checker Console through the shared `review-kit` client (redact-before-wire); `local` enqueues to a transactional outbox so the routing path runs offline, `gcp`/`platform` submit over S2S to `human-review-console`'s service intake and refuse to boot with no `HUMAN_REVIEW_URL`. A deployment can state `MKT_INTEL_REVIEW_ROUTING=off`, and every caller then reports `review_routing: "off"`; a failed hand-off is reported as `failed`, never swallowed | `ports/review_router.py`, `adapters/{local,platform,onprem}/review_router.py`, `adapters/_review_payload.py`, `adapters/controls.py` |

---

## Why `market-intelligence` has no customer-PII surface (R1, C2..C4)
...
- **Public and aggregate inputs only.** A brief is built from grounded public-web deep
  ...
  customer identifier, and no tenant-partitioned customer data (contrast `next-best-action`, the only
  per-customer marketing repo). The practices audit records C2/C3/C4 as **n/a by design**.
- **The guardrail still runs, twice.** Absence of PII is not absence of safety: the `agent-guardrail-gateway` screens INPUT and OUTPUT inside the domain pipeline and again at the ADK model
...
| `market-intelligence` control | Reference regime | What a supervisor looks for |
|---|---|---|
...
with local counsel. The `market-intelligence`-control column is stable across regulators; only the mapping
changes. The sibling `compliance-advisory` and its control-mapping module
