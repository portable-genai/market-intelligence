...
| **P-03** | Data residency (in-country) | Region selected at deploy from a residency allowlist, with per-market overrides (JP asia-northeast1, AU australia-southeast1, SG asia-southeast1), validated to fail fast; regional endpoints; `gcp.resourceLocations` Org Policy; VPC-SC perimeter | `config/settings.yaml` (`markets`), `config.market_profile`, `infra/terraform/variables.tf`, `org_policy.tf`, `vpc_sc.tf` |
