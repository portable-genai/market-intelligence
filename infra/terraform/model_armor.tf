...
# mkt-intel-guardrail`, `adapters/gcp/model_armor_guardrail.py` posts every screen to
# `.../locations/<region>/templates/mkt-intel-guardrail:sanitize*`, iam.tf grants
# `roles/modelarmor.user` and apis.tf enables the API, and nothing created the template. Every
# research and narration screen on a deployment would have been refused by a template that was
# never provisioned. Reused from marketing-compliance-gate/infra/terraform/model_armor.tf, which
# fixed this same defect first.
#
# Control map (SPEC concern):
#   Guardrail screening: prompt injection, jailbreak and the responsible-AI categories on both
#     directions of every research call and every narrated brief.
...
resource "google_model_armor_template" "mkt_intel_guardrail" {
  provider    = google-beta
  project     = var.project_id
  location    = var.region            # the regional endpoint the adapter calls
  template_id = "mkt-intel-guardrail" # matches settings.yaml model_armor.template_id
  ...
  template_metadata {
    ...
    # same variable and the same disclosure. JP (asia-northeast1) research is where it matters.
