...
#
# Control map:
#   Detect, not just record (SPEC 3 AuditSinkPort): DATA_READ logging (logging_worm.tf)
#         records reads, but recording is not detection. These log-based metrics + alert
#         policies SURFACE security-relevant events so an operator is notified, rather than
#         the signal sitting unread in the WORM bucket.
#
# Signals covered:
#   - guardrail_blocks : a Model Armor guardrail BLOCKED decision in the app audit log
#                        (the input/output screen in SPEC 5 working).
#   - sa_key_creation  : an exportable SA key was created (org policy forbids it; alert if it
#                        happens anyway).
...
locals {
  security_metrics = {
    guardrail_blocks = {
      ...
      filter      = "logName=\"projects/${var.project_id}/logs/market-intelligence-audit\" AND jsonPayload.decision=\"blocked\""
    }
    sa_key_creation = {
...
resource "google_monitoring_alert_policy" "security" {
  ...
  project      = var.project_id
  display_name = "Mkt1 security: ${each.key}"
  combiner     = "OR"
  ...
  documentation {
    content   = "Security signal '${each.key}' fired for the Mkt1 market-intelligence service. Investigate the matching entries in Cloud Logging and the WORM audit bucket."
